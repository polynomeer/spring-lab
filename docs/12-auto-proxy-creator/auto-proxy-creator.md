...
38번의 브레이크포인트 히트를 대상 빈별로 정리하면:

| 대상 빈(처리 순서) | 히트 구간 | 관찰 결과 |
| --- | --- | --- |
| `autoProxyTimingConfig`(1번째, `@Configuration` 클래스 자신) | #1~#6 | `postProcessAfterInitialization` → `wrapIfNecessary` → `getAdvicesAndAdvisorsForBean` → `findEligibleAdvisors` → `findCandidateAdvisors` → `findAdvisorBeans`까지는 예상대로 순서대로 진행 |
| ↳ `findAdvisorBeans` 내부에서 아직 없는 `timingAdvisor` 빈을 `getBean()`으로 조회 | #7~#8 | `timingAdvisor`가 그 자리에서 `createBean → initializeBean → postProcessAfterInitialization → wrapIfNecessary`까지 **전체 빈 생성 파이프라인**을 거쳐 즉시 인스턴스화됨 - 이 호출은 **바깥쪽 `findAdvisorBeans`가 아직 리턴하지 않은 채로** 중첩되어 있다(히트 #7의 10번 프레임이 `findAdvisorBeans`) |
| ↳ `timingAdvisor` 자신에 대한 advisor 탐색 | (히트 없음) | `wrapIfNecessary`(#8) 다음 히트가 곧바로 `autoProxyTimingConfig`를 위한 `canApply`(#9)로 넘어감 - `timingAdvisor`에 대한 `getAdvicesAndAdvisorsForBean`은 **단 한 번도 호출되지 않음**. `isInfrastructureClass(bean.getClass())` 체크(361행)가 그 호출보다 먼저 있어, `Advisor` 타입이면 그 자리에서 바로 원본을 반환하기 때문 |
| `autoProxyTimingConfig`의 최종 advisor 판정 | #9~#10 | `canApply(advisor, AutoProxyTimingConfig.class)` → `canApply(pointcut, ...)` 순으로 평가, 매칭 없음(어드바이스 대상 메서드가 없는 설정 클래스) → 프록시 안 됨 |
| `legacyReport`(2번째, 인터페이스 없음) | #11~#20 | `findAdvisorBeans`가 다시 호출되지만(#16) `timingAdvisor` 재생성 없음(이미 싱글톤) → `canApply` true(#17~#18) → `createProxy`(#19) → `DefaultAopProxyFactory#createAopProxy`(#20) |
| `notificationServiceImpl`(3번째, `@MeasureTime` 없음) | #21~#28 | 같은 순서로 `canApply`까지(#27~#28) 진행되지만 결과가 `false` → `getAdvicesAndAdvisorsForBean`이 `DO_NOT_PROXY` 반환 → **`createProxy` 히트 없이** 곧장 다음 빈(#29)으로 진행 |
| `orderServiceImpl`(4번째, 인터페이스 있음) | #29~#38 | `legacyReport`와 완전히 같은 호출 순서로 `createProxy`(#37) → `createAopProxy`(#38)까지 도달 |

같은 `DefaultAopProxyFactory#createAopProxy`(히트 #20, #38)에서 멈췄지만 그 뒤의 프로그램 출력은 갈린다 — `legacyReport`는 `AopUtils.isCglibProxy()`가 `true`, `orderServiceImpl`은 `AopUtils.isAopProxy()`만 `true`(JDK 프록시)다. 분기 자체는 이 메서드 안(소스로 재확인: `config.isProxyTargetClass() || !config.hasUserSuppliedInterfaces()`면 대상 클래스가 인터페이스/프록시/람다가 아닌 한 CGLIB, 그렇지 않으면 JDK)에 있지만, 두 빈이 정확히 같은 코드 경로를 타다가 이 지점에서만 갈린다는 것을 나란히 보는 게 이번 세션의 핵심이었다.
