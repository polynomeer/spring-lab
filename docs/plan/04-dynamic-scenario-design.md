...
[`03-learning-dashboard-design.md`](03-learning-dashboard-design.md)가 만든 대시보드는 지금까지 시나리오 7개(빈 생명주기, AOP 자동 프록시, 트랜잭션 전파, DispatcherServlet 흐름, 이벤트 멀티캐스트, MVC 예외 우선순위, 조건 평가 리포트)를 전부 `ScenarioCatalog`에 **자바 코드로 하드코딩**해 뒀다 — 어느 Gradle 모듈의 어느 `main()` 클래스를, 어느 브레이크포인트 스펙으로, 어느 손으로 짠 해석기로 보여줄지가 전부 소스에 박혀 있다. 새 시나리오를 하나 추가하려면 새 실험 모듈 + `main()` 진입점 + 스펙 파일 + 해석기 클래스까지 매번 손으로 만들고 커밋해야 한다.
...
`03`번 문서 9번 절은 명시적으로 "강한 인터페이스로 플러그인화를 미리 설계하지 않는다 — 실제로 두세 개 더 늘려 봐야 진짜 필요한 추상화가 보인다"고 못 박아 뒀다. 이 문서는 정확히 그 "플러그인화"를 하겠다는 것이므로, 왜 지금은 타당한지 먼저 밝혀 둔다: 그 원칙이 말한 "두세 개"를 이미 훌쩍 넘어 7개 시나리오를 만들었고, 그 7개 모두 정확히 같은 네 가지 재료(모듈 경로, main 클래스, 브레이크포인트 스펙, 해석기)로 조립된다는 패턴이 반복적으로 확인됐다 — `ScenarioCatalog.register()` 호출 7번이 그 증거다. 지금 일반화하는 것은 조급한 추상화가 아니라, 이미 충분히 검증된 패턴을 뒤늦게 데이터로 옮기는 것이다.
...
- 기존 7개 시나리오는 애플리케이션 시작 시 `CommandLineRunner`(또는 `data.sql`)로 이 테이블에 시드 데이터로 들어간다 — `ScenarioCatalog`의 `register()` 호출 7번이 그대로 시드 레코드 7행으로 옮겨진다. **기존 6종 해석기는 그대로 자바 클래스로 남고**, `interpreterKind` 컬럼(문자열 enum)으로 시드 레코드에만 연결해 둔다 — 사용자 정의 시나리오는 항상 `interpreterKind = NONE`.
...
- **기존 Lab을 템플릿으로 복제** - "새 시나리오 작성" 시 기존 7개 중 하나를 골라 그 소스를 읽어와 편집기에 미리 채워 주는 "복제해서 수정" 버튼. 백지에서 시작하지 않아도 되게 만드는 값싼 UX 개선.
- **소스 코드 안의 인라인 라벨 주석** - `// @dashboard-label: "빈 생성 시작"`처럼 브레이크포인트 지점 바로 위에 주석을 달아 두면, 서버가 컴파일 전에 그 주석을 파싱해서 손으로 짠 해석기 없이도 최소한의 의미 있는 라벨/색을 자동으로 붙여 준다. 라벨을 별도 JSON 필드로 관리하지 않고 코드 자체에 두는 것 - "정보를 어디에 저장하느냐가 그 정보를 다루는 방식을 결정한다"는 이 저장소가 [50번 문서](../50-placeholder-resolution/placeholder-resolution.md)에서 확인한 원칙과 같은 결이다.
...
1단계: DB 기반 카탈로그 (컴파일 없음)
  ...
  - 기존 7개 시나리오를 시드 데이터로 이관(spec 파일 내용을 컬럼으로 복사)
  ...
  - 검증: 기존 7개 시나리오가 DB를 거쳐도 그대로 동작하는지 회귀 확인,
