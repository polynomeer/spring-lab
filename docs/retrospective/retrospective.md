...
**선택 4주에서**
- ~~17주차에서 미룬 "웹 서버가 실제로 언제 뜨는가"는 자동 설정(18주차) 없이는 관찰할 수 없어서 미뤘는데, 20주차에서 실제 웹 자동 설정을 통합했지만 이 관찰 자체는 별도로 다시 다루지 않았다.~~ — 18번 절에 적었듯 이후에 마쳤다. 이것으로 이 목록(7번 절 "남겨 둔 질문") 전체가 채워졌다.
...
## 10. Mini Order Platform 다음 — 상품 도메인 추가

카탈로그 16번 절이 이 캡스톤의 도메인으로 회원/상품/주문/결제/알림/감사 로그 여섯 개를 나열해 뒀지만, 9번 절이 정리한 Phase 1~6의 21개 세부 기법 중 어느 것도 실제로 상품 도메인을 요구하지 않았다 - 그래서 캡스톤이 "완료"된 뒤에도 상품만 빈 자리로 남아 있었다. 카탈로그가 요구하지 않는 확장이라 별도 Phase 번호를 붙이지 않고, `sample-app/mini-order-platform`에 `product` 패키지와 `OrderLineItem`을 더하는 식으로 진행했다 - 상세 설계는 [`docs/24-mini-order-platform/mini-order-platform.md`](../24-mini-order-platform/mini-order-platform.md) 11번 절에 있다.

### 10.1 숫자로 보는 지금

- 모듈 수는 그대로 35개다 - 새 모듈을 만들지 않고 기존 `sample-app/mini-order-platform` 안에 패키지만 추가했다.
- 이 모듈의 테스트가 43개에서 54개로 늘었고, 저장소 전체 자동화 테스트는 9번 절의 326개에서 337개가 됐다.

### 10.2 가장 크게 배운 것 — 캡스톤이 조용히 미뤄 뒀던 질문

`OrderPlacementService`는 Phase 3(트랜잭션)부터 Phase 6(Boot)까지 계속 `long amountWon`을 그대로 파라미터로 받아 왔다 - 결제/트랜잭션/이벤트/Boot 메커니즘을 검증하는 데는 그걸로 충분했기 때문에 아무도 "그 금액이 어디서 왔는가"를 묻지 않았다. 상품 도메인을 실제로 붙이면서야 그 질문이 드러났다 - 클라이언트가 금액을 직접 부를 수 있게 하는 API는 그 자체로 보안 결함이다. `OrderItemRequest`가 상품 ID와 수량만 받고 서버가 `Product` 테이블에서 가격을 계산하도록 바꾼 게 이번 확장의 핵심이었다.

재고 차감에서도 비슷한 종류의 함정을 피해 갔다 - "조회 후 갱신"(재고를 읽고, 자바에서 빼고, 다시 쓰는) 대신 `UPDATE products SET stock = stock - ? WHERE id = ? AND stock >= ?` 한 문장으로 조회·조건 검사·갱신을 원자적으로 합쳤다. 이건 완전히 새로운 개념이 아니라, 4주차(빈 생명주기)부터 반복돼 온 2번 절의 패턴("정교한 판단보다 예측 가능한 순서")과 같은 결의 선택이다 - 애플리케이션 코드에서 락을 잡고 판단하는 대신, DB 엔진이 이미 원자적으로 보장하는 단위(단일 UPDATE 문)에 판단 자체를 맡겼다.

### 10.3 반복된 것 — 이후 확장이 이전 테스트를 깨뜨림

`OrderPlacementService.placeOrder()`의 시그니처를 바꾸자 Phase 3/4/5에서 이미 짜 둔 테스트가 전부 컴파일조차 안 됐다. 9번 절이 "Phase 5가 Phase 3의 단언 하나를 깨뜨렸다"고 기록해 둔 것과 정확히 같은 종류의 파급이 이번엔 훨씬 큰 규모로(테스트 파일 4개) 일어났다 - 카탈로그의 32개 프로젝트가 서로 독립된 모듈이었던 것과 달리, 이 캡스톤은 하나의 애플리케이션이 계속 성장하는 구조라 이런 파급이 구조적으로 반복될 수밖에 없다는 걸 이번에 다시 확인했다. 각 테스트에 상품을 미리 심어 두는 준비 단계를 추가하는 것으로 고쳤고, 실패하는 컴파일 오류가 정확히 어디를 고쳐야 하는지 알려 줘서 수정 자체는 어렵지 않았다 - 오히려 "타입이 강제하는 컴파일 오류 덕분에 놓친 호출부가 하나도 없었다"는 게 이번에 체감한 안전망이었다.

## 11. 남겨 둔 질문으로 되돌아가기 — CGLIB 상당 서브클래스 프록시

7번 절이 "핵심 메커니즘을 이해하는 데는 필요 없었다"며 미뤄 둔 mini 구현들의 생략 목록 중 첫 번째를 채웠다: `mini-spring/mini-aop`에 `MiniSubclassProxyFactory`를 추가해서, 인터페이스가 없는 대상도 프록시할 수 있게 했다. 상세 내용은 [`docs/11-proxy-interceptor/proxy-interceptor.md`](../11-proxy-interceptor/proxy-interceptor.md)의 후기(5·8·10·11·12번 절)에 그대로 반영했다 - 이 절은 그중 반복해서 배울 만한 것만 추린다.

**바이트코드는 라이브러리에 맡기고, 우리가 배우려던 것만 새로 짰다.** 원래 이 문서(그리고 11주차 문서 10번 절)는 "CGLIB 자체를 반쪽으로 재구현하는 건 바이트코드 생성 영역이라 범위 밖"이라고 판단했었다. 실제로 만들어 보니 그 판단은 절반만 맞았다 - `net.bytebuddy:byte-buddy`(실제 Spring이 CGLIB을 벤더링해서 쓰는 것과 같은 선택)로 바이트코드 생성 자체를 처리하고 나니, 남는 일은 11주차에 이미 만들어 둔 `MethodInterceptor`/`ReflectiveMethodInvocation`을 그대로 재사용하는 것뿐이었다 - 새 인터셉터 체인 로직은 한 줄도 필요 없었다. "학습 목표 밖"이라고 적었던 것 중 실제로 학습 목표 밖이었던 부분(바이트코드 생성)과, 그냥 "어려워 보여서" 건드리지 않았을 뿐 사실은 기존 코드로 충분했던 부분(인터셉터 체인 재사용)이 섞여 있었다는 뜻이다 - 미뤄 둔 이유를 다시 검증해 보지 않으면 이 둘을 구분할 수 없다.
...
## 18. 남겨 둔 질문 — 17주차 웹 서버 기동 시점 관찰 (그리고 7번 절의 마지막 항목)

7번 절 "남겨 둔 질문"에 남아 있던 정말 마지막 항목을 채웠다: `experiments/spring-application-lifecycle`에 실제 `WebApplicationType.SERVLET` 실행을 추가해서, 내장 톰캣이 실제로 언제 요청을 받기 시작하는지, 그 순간에 이 애플리케이션의 평범한 빈들은 이미 준비돼 있는지를 직접 관찰했다. 상세 내용은 [`docs/17-spring-application/spring-application.md`](../17-spring-application/spring-application.md)의 후기(5·6·8·11·12번 절)에 반영했다 - 이 절은 그중 반복해서 배울 만한 것만 추린다.

**가설이 틀렸다는 걸 실행이 먼저 알려줬고, 소스가 없어서 바이트코드가 그 이유를 확인해 줬다.** "포트가 열리는 시점"과 "이 애플리케이션의 빈이 준비되는 시점"이 서로 다른 refresh() 단계일 거라 예상하고 테스트를 짰는데, 실행해 보니 `markerBeanAvailable` 단언이 실패했다 - 실제로는 둘 다 이미 참이었다. 이 저장소에는 `spring-boot`의 소스 jar가 없어서(9번 절이 이미 밝혀 둔 제약) 공식 소스로 이유를 확인할 수 없었는데, `spring-boot-3.5.0.jar`를 `javap -c`로 직접 디컴파일해서 `ServletWebServerApplicationContext#createWebServer()`가 웹 서버를 직접 시작하지 않고 `WebServerStartStopLifecycle`이라는 `SmartLifecycle` 빈을 등록만 해 둔다는 것, 그 빈의 `start()`(실제 시작 지점)가 `refresh()`의 마지막 단계(`finishRefresh()`)에서 호출된다는 것을 바이트코드로 직접 확인했다. "소스를 못 읽으면 못 배운다"가 아니라 "실행과 바이트코드로도 충분히 확인할 수 있다"는 걸 보여준 사례다.

**"확장점을 좁게 쪼갠다"는 원칙이 여기서는 "기존 확장점의 순서에 올라탄다"로 나타났다.** 웹 서버를 애플리케이션의 다른 빈들보다 먼저 시작하면, 아직 준비 안 된 컨트롤러/서비스로 실제 요청이 들어오는 위험한 창이 생긴다. Spring Boot는 이 문제를 위한 별도의 순서 제어 로직을 만드는 대신, 이미 있던 `Lifecycle`/`SmartLifecycle` 확장점에 웹 서버 시작을 태워 보냈다 - `refresh()`의 표준 순서(모든 싱글턴 생성 → `SmartLifecycle` 시작)가 "애플리케이션이 완전히 준비된 뒤에만 포트를 연다"는 안전성을 공짜로 보장해 준다. 2번 절이 20주 동안 반복해서 확인한 패턴("확장점은 좁고 합성 가능하게 쪼갠다")이, 이번엔 "새 요구사항이 생겼을 때 새 메커니즘을 만들지 않고 기존 확장점을 재사용한다"는 형태로 한 번 더 나타난 셈이다.

저장소 전체 자동화 테스트는 368개에서 369개가 됐다.

**이것으로 `docs/retrospective/retrospective.md` 7번 절 "남겨 둔 질문"에 적혀 있던 모든 항목(핵심 16주의 CGLIB 상당 서브클래스 프록시·NESTED 전파·JSON 역직렬화·`@ControllerAdvice` 전역 예외 처리·인자 리졸버 캐싱·ASM 기반 컴포넌트 스캔 비교, 선택 4주의 Micrometer 연동·웹 서버 기동 시점 관찰)이 전부 채워졌다.** 20주 로드맵(핵심 16주 + 선택 4주)과 그 이후 확장한 모든 주제를 통틀어, 이 회고 문서가 스스로 남겨 뒀던 미완료 항목은 이제 하나도 남지 않았다.
